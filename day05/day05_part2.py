...
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2023/12/05 07:46:42 by Dawnaur           #+#    #+#              #
#    Updated: 2023/12/05 08:07:11 by Dawnaur          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
	...
	for line in input_file:
		if (len(line) > 1):
			if (task == 0):
				# get seeds
				seeds = []
				splitted = line.split(":")[1][1:-1].split(" ")
				for i in range(0, len(splitted), 2):
					for seed in range(int(splitted[i]), int(splitted[i]) + int(splitted[i + 1])):
						seeds.append(seed)
#				print(seeds)
			else:
				if line[0].isdigit():
